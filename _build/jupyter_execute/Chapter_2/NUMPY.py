...
# In this chapter we will be discussing the advatages to using the *NumPy* package to analyze large datasets. Numpy is a useful tool that can help store and wrangle homogeneous data. This means data that the data are of the same data type such as all `floats` or all `integers`.
...
# You may want to look at a slice of cloumns or a slice of rows. You can do so using `start_index:stop_index` for either row or columns to select your slice.
...
# We can also index arrays using booleans. When we use Booleans, we can think of this as *filtering* the array by applying a condition to our array. The syntax for this type of indexing would be `new_array = original_array[condition]`.
...
# Index values greater than 5 from our array 
condition = (array1 > 5)
...
# The list of lists format is not wrong per say, but it makes working with the data more difficult. For example, if you were trying to add the numbers of the two lists together, simply adding the lists would only append one list at the end of the other. However, if you add two numpy arrays together, the values of voth arrays will be summed. 
